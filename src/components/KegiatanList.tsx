...
export default function KegiatanList() {
  ...
  // Filter by judul (case-insensitive substring)
  const filteredItems = items.filter((it) => {
    const q = filterTitle.trim().toLowerCase()
    if (!q) return true
    return (it.judul || '').toLowerCase().includes(q)
  })
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-start sm:items-end gap-3 sm:gap-4">
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <div className="flex flex-col w-full">
            <label className="text-xs text-gray-600 dark:text-gray-400 mb-1">Judul</label>
            <input
              type="text"
              value={filterTitle}
              onChange={(e) => setFilterTitle(e.target.value)}
              placeholder="Cari judul kegiatan..."
              aria-label="Filter judul kegiatan"
              title="Ketik untuk memfilter judul"
              className="w-full sm:w-72 border border-gray-300 dark:border-gray-700 rounded-md px-3 py-2 text-sm bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
            />
          </div>
        </div>
